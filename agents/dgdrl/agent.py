...
from agents.models.diffusion.diffusion import LatentDiffusionModel
...
class Agent():
    def __init__(self, training_config, augmentation_config, vehicle_measurements_config, waypoints_config, image_config, critic_config, actor_config, memory_config, control_config, maximum_speed, experiment_path, init_memory):
        ...
        self.latentdiffusionmodel=LatentDiffusionModel(self.state_size,1000,training_config['lr'],training_config['weight_decay'],self.experiment_path,self.device)
        ...
        if self.automatic_entropy_tuning:
            ...
            self.alpha_optim = torch.optim.Adam([self.log_alpha], lr=training_config['lr_alpha'])
    ...
    def choose_action(self, obs, step, training=True):
        ...
        if self.action_ctn % self.repeat_action == 0:
            ...
            obs = self.convert_obs_into_torch(obs=obs, unsqueeze=True)
    ...
    def filter_obs(self, obs):
        obs_ = {}
        resized_image = cv2.resize(obs['image']['data'], self.obs_info['image']['shape'][1:3], interpolation=cv2.INTER_AREA)
        obs_['image'] = np.einsum('kij->jki', resized_image)
        obs_['waypoints'] = np.array(obs['waypoints']['location'])[0:self.num_waypoints, 0:2].reshape(self.num_waypoints, 2)
        obs_speed = np.array(obs['speed']['speed'][0] / self.maximum_speed, dtype=np.float32).reshape(1)
        obs_steer = np.array(obs['control']['steer'][0]).reshape(1)
        obs_['vehicle_measurements'] = np.concatenate([obs_speed, obs_steer]).reshape(2)
        obs_['desired_speed'] = np.array(obs['desired_speed'] / self.maximum_speed, dtype=np.float32).reshape(1)
    ...
    def train(self, step):
        self.train_ctn += 1
        metrics = dict()
        if self.train_ctn < 1500:
            return metrics

        # sample batch from memory.
        obs_batch, action_batch, reward_batch, discount_batch, next_obs_batch, done_batch = tuple(next(self.replay_iter))
        #print("reward_batch",reward_batch)
        obs_batch = self.convert_obs_into_device(obs_batch)
        next_obs_batch = self.convert_obs_into_device(next_obs_batch)

        if self.use_aug:
            obs_batch = self.augment_obs(obs=obs_batch)
            next_obs_batch = self.augment_obs(obs=next_obs_batch)
        ...
        # critic networks.
        metrics.update(self.update_critics(obs_batch=obs_batch, action_batch=action_batch, reward_batch=reward_batch, discount_batch=discount_batch,next_obs_batch=next_obs_batch, done_batch=done_batch))
    ...
    def update_critics(self, obs_batch, action_batch, reward_batch, discount_batch, next_obs_batch, done_batch):
        ...
        with torch.no_grad():
            ...
            min_q_next_target = torch.min(q1_next_target, q2_next_target) - self.alpha * next_state_log_prob
    ...
    def update_policy(self, obs_batch):
        ...
        #self.latentdiffusionmodel.train()
        timestep = torch.randint(0, self.latentdiffusionmodel.num_timesteps, (1,)).item()
        h_t_pred, noise,epsilon_pred = self.latentdiffusionmodel(state_batch,timestep)
        lossLDM = self.latentdiffusionmodel.compute_diffusion_loss(epsilon_pred,noise)


        actions_speed_h_t_pred, log_prob_speed_h_t_pred, _, _ = self.policy_speed.sample(h_t_pred)
        actions_steer_h_t_pred, log_prob_steer_h_t_pred, _ = self.policy_steer.sample(h_t_pred)
        actions_h_t_pred = torch.cat([actions_speed_h_t_pred, actions_steer_h_t_pred], dim=1)
        # policy steer
        ...
        actions_h_t_pred_cpu = actions_h_t_pred.detach().cpu().numpy()
        actions_cpu = actions.detach().cpu().numpy()
        actions_h_t_pred_cpu = softmax(actions_h_t_pred_cpu, axis=1)
        actions_cpu = softmax(actions_cpu, axis=1)
        kl_divergence = entropy(actions_h_t_pred_cpu.T, actions_cpu.T)
        #print(kl_divergence)
        kl_divergence = torch.tensor(kl_divergence, dtype=torch.float32)
        policy_loss = ((self.alpha * log_prob) - min_q).mean() + 0.1 * (kl_divergence.mean())
        self.policy_speed.optimizer.zero_grad(set_to_none=True)
        self.policy_steer.optimizer.zero_grad(set_to_none=True)
        policy_loss.backward()
        #print(policy_loss)
        self.latentdiffusionmodel.optimizer.zero_grad(set_to_none=True)
        lossLDM.backward()
        torch.nn.utils.clip_grad_norm_(self.latentdiffusionmodel.parameters(), self.grad_clip)
        self.latentdiffusionmodel.optimizer.step()
        self.latentdiffusionmodel.scheduler.step(lossLDM) 
        ...
        metrics['diffusion_loss'] = round(lossLDM.item(), 4)
        #metrics['diffusion_h'] =  np.round(state_batch[0].detach().cpu().numpy(), 4)
        #metrics['diffusion_h_hat'] = np.round(h_t_pred[0].detach().cpu().numpy(), 4)
        return metrics
    ...
    def set_eval_mode(self):
        ...
        self.latentdiffusionmodel.eval()
    ...
    def save_models(self, save_memory=False):
        ...
        self.latentdiffusionmodel.save_checkpoint()
    ...
    def load_models(self, save_memory=False):
        ...
        self.latentdiffusionmodel.load_checkpoint()
